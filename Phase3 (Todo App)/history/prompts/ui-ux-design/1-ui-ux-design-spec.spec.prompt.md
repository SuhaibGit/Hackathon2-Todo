...
user: user
command: /sp.specify
labels: [specification, ui, ux, design, todo-app, frontend]
links:
  ...
  pr: null
files:
  - specs/ui-ux-design/spec.md
  - specs/ui-ux-design/checklists/requirements.md
tests:
  - none
---
...
Feature: Structural UI + UX Design & Implementation – Todo App MVP
...
- As a user I can switch between light and dark mode (persisted preference)
- As a logged-in user I see my task list in a clean card/list format
- As a logged-in user with no tasks I see a motivating empty state + create button
- As a user I can open a form/modal to create a new task (title + optional description)
...
Acceptance Criteria:
- Header is fixed/sticky on scroll (mobile) or static (desktop)
- Dark/light mode toggle works instantly + persists (localStorage)
- Task items show: checkbox (left), title (strikethrough when done), actions (edit/delete)
- Create/Edit form validates title (required, 1–200 chars)
- Empty state is centered, motivational, includes big + button
- Responsive breakpoints: <640px (mobile), 640–1024px, >1024px
- Uses Tailwind CSS exclusively – no custom CSS files
- Follows accessibility basics: semantic HTML, focus states, aria-labels where needed
- Uses mock data or localStorage for tasks during this phase

Design guidelines (must follow):
- Color palette:
  Light: bg-gray-50, text-gray-900, primary-blue-600, green-500 completed
  Dark: bg-gray-900, text-gray-100, primary-blue-400, green-400 completed
- Font: Inter (or system-ui stack)
- Spacing: generous (p-4 → p-8, gap-4 → gap-6)
- Cards: subtle shadow, rounded-lg, border in light mode
- Buttons: rounded-md, hover states, primary blue
- Floating action button on mobile (bottom-right)

Non-Goals (defer to later phases):
- Advanced filters/sorting/search
...
- TypeScript
- Tailwind CSS
- shadcn/ui or Radix UI primitives (optional but recommended for accessibility)
- Better Auth session for user state

Mock strategy:
- Use localStorage to persist tasks per user (simulate multi-user)
- Or simple in-memory array during development

Requested output structure:
1. Read relevant specs (@specs/ui/pages.md, @specs/ui/components.md, @specs/features/task-crud.md)
2. Visual & structural description summary (header, main, footer, mobile vs desktop)
3. Component tree / file structure proposal
4. Numbered implementation plan (8–14 small tasks)
   - Task title
   - Files affected
   - Primary agent (FrontendEngineer)
   - Dependencies
   - Acceptance criteria
5. Definition of Done for this UI/UX phase
6. Next recommended command (/sp.plan or /sp.specify backend-connection)

Be opinionated but practical — aim for a calm, professional, fast-feeling Todo UI.
...
- ✅ Impact: Created comprehensive specification for UI/UX design with user stories, requirements, and implementation plan
- 🧪 Tests: none
- 📁 Files: Created specs/ui-ux-design/spec.md and specs/ui-ux-design/checklists/requirements.md
- 🔁 Next prompts: /sp.plan to generate detailed implementation plan
